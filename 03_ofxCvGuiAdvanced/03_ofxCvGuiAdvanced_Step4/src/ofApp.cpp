...
void ofApp::setup(){
    ...
    auto widgets = this->gui.addScroll("Widgets");
    ...
    for(auto & sphere : this->spheres) {
        widgets->add(ofxCvGui::Widgets::Title::make("Sphere"));
        
        for(int i=0; i<3; i++) {
            widgets->add(ofxCvGui::Widgets::Slider::make(sphere.xyz[i]));
        }
        
        widgets->add(ofxCvGui::Widgets::Slider::make(sphere.radius));
        widgets->add(ofxCvGui::Widgets::Slider::make(sphere.hue));
    }
}
